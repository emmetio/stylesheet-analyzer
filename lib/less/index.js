...
import extendRule, { getExtend } from './extend';
...
export default class LESSStylesheet {
	...
	transform(dependencies) {
		...
		// A set of `:extend()` instructions found during transform
		const extend = {};
		...
		const next = (node, scope) => {
			if (Array.isArray(node)) {
				...
				const extendData = getExtend(node, scope);
				if (extendData) {
					mergeExtends(extend, extendData);
				}
		...
		// Apply extend’s
		Object.keys(extend).forEach(sel => {
			extend[sel].forEach(data => extendRule(sel, data));
		});
...
function mergeExtends(collection, extendData) {
	extendData && Object.keys(extendData).forEach(sel => {
		if (sel in collection) {
			collection[sel] = collection[sel].concat(extendData[sel]);
		} else {
			collection[sel] = extendData[sel];
		}
	});
