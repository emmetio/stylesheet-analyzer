 ...
 */
function applyExtend(node, selector, graph, visited) {
		...
		if (extended) {
			...
			if (graph.has(item.selector)) {
				...
				if (visited.has(item.node)) {
					// Node’s extends already processed, apply extended node
					// immediately. Otherwise wait until visitor enters node
					// TODO pos-pone execution
					applyExtend(node, extended, graph, visited);
				}
			}
		}
	});
