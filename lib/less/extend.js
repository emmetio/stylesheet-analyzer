...
export default function extend(stylesheet) {
	...
	allNodes.forEach(node => {
		...
		if (ext && ext.length) {
			node.selectors.forEach(sel => applyExtend2(sel, selGraph, lookup));
		}
	});
 ...
 * @param {Node[]} nodes
 */
function applyExtend2(selector, graph, lookup, guard) {
	...
	graph.get(selector).forEach(item => {
		// console.log('extend %s with %s', item.selector, selector);
		...
		if (extended) {
			...
			if (graph.has(item.selector)) {
				...
				items.forEach(subitem => {
					...
					if (lookup.has(subitem.node) && !guard.has(subitem.node)) {
						...
						applyExtend2(extended, graph, lookup, guard);
					}
				});
			}
		}
